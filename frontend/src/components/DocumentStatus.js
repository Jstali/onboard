...
const DocumentStatus = ({
  ...
  const fetchValidation = useCallback(async () => {
    try {
      ...
      const response = await axios.get(
        `/api/documents/validation/${employeeId}`,
  ...
  const handleDeleteDocument = async (documentType) => {
    ...
    try {
      ...
      const documents = await axios.get(
        `/api/hr/document-collection/employee/${employeeId}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      ...
      if (docToDelete) {
        await axios.delete(
          `/api/hr/document-collection/${docToDelete.id}`,
          { headers: { Authorization: `Bearer ${token}` } }
        );
  ...
  const handleReplaceDocument = async (
    ...
    try {
      ...
      const token = localStorage.getItem("token");
      await axios.post(
        `/api/documents/upload/${employeeId}`,
        formData,
        {
          headers: {
            ...
            <div className="border rounded-lg overflow-hidden">
              ...
              (selectedFile.file_type?.startsWith("image/") ||
                ...
                <div className="text-center">
                  <img
                    src={`/api/documents/preview/${selectedFile.id}`}
