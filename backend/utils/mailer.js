...
async function sendLeaveRequestToManager(managerEmail, leaveRequest) {
  const mailOptions = {
    ...
    text: `Leave Request Details:\n\nEmployee: ${leaveRequest.employeeName}\nLeave Type: ${leaveRequest.leaveType}\nFrom: ${leaveRequest.fromDate}\nTo: ${leaveRequest.toDate || 'Single Day'}\nTotal Days: ${leaveRequest.totalDays}\nReason: ${leaveRequest.reason}\n\nPlease approve or reject this request by clicking the links below:\n\nApprove: http://localhost:5001/api/leave/approve/${leaveRequest.id}?action=approve&token=${leaveRequest.approvalToken}\nReject: http://localhost:5001/api/leave/approve/${leaveRequest.id}?action=reject&token=${leaveRequest.approvalToken}`,
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <div style="background-color: #f5f5f5; padding: 20px; border-radius: 8px; margin: 20px 0;">
          ...
          <table style="width: 100%; border-collapse: collapse;">
            <tr><td style="padding: 8px 0; border-bottom: 1px solid #ddd;"><strong>Employee:</strong></td><td style="padding: 8px 0; border-bottom: 1px solid #ddd;">${leaveRequest.employeeName}</td></tr>
            <tr><td style="padding: 8px 0; border-bottom: 1px solid #ddd;"><strong>Leave Type:</strong></td><td style="padding: 8px 0; border-bottom: 1px solid #ddd;">${leaveRequest.leaveType}</td></tr>
            <tr><td style="padding: 8px 0; border-bottom: 1px solid #ddd;"><strong>From Date:</strong></td><td style="padding: 8px 0; border-bottom: 1px solid #ddd;">${leaveRequest.fromDate}</td></tr>
            <tr><td style="padding: 8px 0; border-bottom: 1px solid #ddd;"><strong>To Date:</strong></td><td style="padding: 8px 0; border-bottom: 1px solid #ddd;">${leaveRequest.toDate || 'Single Day'}</td></tr>
            <tr><td style="padding: 8px 0; border-bottom: 1px solid #ddd;"><strong>Total Days:</strong></td><td style="padding: 8px 0; border-bottom: 1px solid #ddd;">${leaveRequest.totalDays}</td></tr>
            <tr><td style="padding: 8px 0; border-bottom: 1px solid #ddd;"><strong>Reason:</strong></td><td style="padding: 8px 0; border-bottom: 1px solid #ddd;">${leaveRequest.reason}</td></tr>
          </table>
        </div>
        
        <div style="text-align: center; margin: 30px 0;">
          <a href="http://localhost:5001/api/leave/approve/${leaveRequest.id}?action=approve&token=${leaveRequest.approvalToken}" 
          ...
          <a href="http://localhost:5001/api/leave/approve/${leaveRequest.id}?action=reject&token=${leaveRequest.approvalToken}" 
...
async function sendManagerApprovalToHR(hrEmail, leaveRequest, managerName) {
  ...
  try {
    ...
    console.error("❌ Failed to send manager approval notification to HR:", err);
    return false;
  }
}

async function sendLeaveApprovalToEmployee(employeeEmail, leaveRequest, status, approverName) {
  const mailOptions = {
    from: process.env.EMAIL_USER,
    to: employeeEmail,
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <h2 style="color: #333;">Leave Request ${status === 'approved' ? 'Approved' : 'Rejected'}</h2>
        <p>Your leave request has been <strong>${status}</strong> by ${approverName}.</p>
        
        <div style="background-color: ${status === 'approved' ? '#d4edda' : '#f8d7da'}; border: 1px solid ${status === 'approved' ? '#c3e6cb' : '#f5c6cb'}; padding: 20px; border-radius: 8px; margin: 20px 0;">
          <h3 style="color: ${status === 'approved' ? '#155724' : '#721c24'}; margin-top: 0;">Request Details</h3>
          <table style="width: 100%; border-collapse: collapse;">
            <tr><td style="padding: 8px 0; border-bottom: 1px solid #ddd;"><strong>Leave Type:</strong></td><td style="padding: 8px 0; border-bottom: 1px solid #ddd;">${leaveRequest.leaveType}</td></tr>
            <tr><td style="padding: 8px 0; border-bottom: 1px solid #ddd;"><strong>From Date:</strong></td><td style="padding: 8px 0; border-bottom: 1px solid #ddd;">${leaveRequest.fromDate}</td></tr>
            <tr><td style="padding: 8px 0; border-bottom: 1px solid #ddd;"><strong>To Date:</strong></td><td style="padding: 0; border-bottom: 1px solid #ddd;">${leaveRequest.toDate || 'Single Day'}</td></tr>
            <tr><td style="padding: 8px 0; border-bottom: 1px solid #ddd;"><strong>Total Days:</strong></td><td style="padding: 8px 0; border-bottom: 1px solid #ddd;">${leaveRequest.totalDays}</td></tr>
            <tr><td style="padding: 8px 0; border-bottom: 1px solid #ddd;"><strong>Reason:</strong></td><td style="padding: 8px 0; border-bottom: 1px solid #ddd;">${leaveRequest.reason}</td></tr>
            <tr><td style="padding: 8px 0; border-bottom: 1px solid #ddd;"><strong>Status:</strong></td><td style="padding: 8px 0; border-bottom: 1px solid #ddd;"><span style="color: ${status === 'approved' ? '#155724' : '#721c24'}; font-weight: bold;">${status.toUpperCase()}</span></td></tr>
  ...
  try {
    await transporter.sendMail(mailOptions);
    console.log("✅ Leave approval notification sent to employee: " + employeeEmail);
    return true;
  } catch (err) {
    console.error("❌ Failed to send leave approval notification to employee:", err);
    return false;
  }
}
